...
interface CategoryPageProps {
    ...
    title: string;
    description: string;
    // products prop is no longer used for data, but kept for compatibility if needed (ignored)
    products?: any[];
}

export function DynamicCategoryPage({ categorySlug, title, description }: CategoryPageProps) {
        ...
        const fetchProducts = async () => {
            try {
                ...
                const response = await fetch(
                    `/api/products?category=${categorySlug}&isActive=true&_t=${Date.now()}`,
                    {
                        cache: "no-store",
                        headers: {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-black">
            <DynamicNavbar currentPath={`/category/${categorySlug}`} />

            {/* 3D Abstract Hero Strip */}
            <div className="relative h-[40vh] overflow-hidden bg-black flex items-center justify-center">
                <div className="absolute inset-0 bg-gradient-to-b from-primary/10 to-transparent opacity-50" />
                <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-overlay" />

                <div className="relative z-10 text-center px-4 max-w-4xl">
                    <span className="block font-mono text-primary text-sm uppercase tracking-[0.2em] mb-4 animate-pulse">Collection</span>
            ...
            <div className="container mx-auto px-4 py-8">
                {/* Filters Bar (Simplified for Generic Categories) */}
